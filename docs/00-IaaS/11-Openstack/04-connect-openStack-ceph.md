...
![connect1](assets/04-connect-openStack-ceph/connect1.png)

- 테스트를 위한 가벼운 OS인 cirros 로 vm을 만들어 테스트를 해보자.
- 주소: https://download.cirros-cloud.net/
- 하나를 골라 다운로드한다.

![connect2](assets/04-connect-openStack-ceph/connect2.png)

- horizon에서 프로젝트 > Compute > 이미지
- 이미지 생성 클릭

![connect3](assets/04-connect-openStack-ceph/connect3.png)
...
![connect4](assets/04-connect-openStack-ceph/connect4.png)

- 업로드하고 ceph의 Block > 이미지 들어가보면 images 데이터풀에 업로드 된 것을 볼 수 있다.

### Flavor 생성

![connect5](assets/04-connect-openStack-ceph/connect5.png)

- 관리 > Compute > Flavor
- Flavor 생성 클릭

![connect6](assets/04-connect-openStack-ceph/connect6.png)
...
![connect7](assets/04-connect-openStack-ceph/connect7.png)

- 프로젝트 > 네트워크 > 네트워크
- 네트워크 생성 클릭

![connect8](assets/04-connect-openStack-ceph/connect8.png)

![connect9](assets/04-connect-openStack-ceph/connect9.png)
...
![connect10](assets/04-connect-openStack-ceph/connect10.png)

- 프로젝트 > Compute > 인스턴스
- 인스턴스 시작 클릭

![connect11](assets/04-connect-openStack-ceph/connect11.png)

- 인스턴스 이름 지정

![connect12](assets/04-connect-openStack-ceph/connect12.png)
...
![connect13](assets/04-connect-openStack-ceph/connect13.png)

- 아까 만든 Flavor 의 화살표를 클릭해서 할당됨으로 옮기기

![connect14](assets/04-connect-openStack-ceph/connect14.png)

- 이건 아마 이미 할당됨 일텐데 혹시 사용 가능에 있다면 화살표 눌러서 아까 만든 네트워크 할당됨으로 옮기기
- 인스턴스 시작 클릭

![connect15](assets/04-connect-openStack-ceph/connect15.png)

- 조금 기다리면 성공적으로 생성된 것을 볼 수 있다.

### 최종 확인

![connect16](assets/04-connect-openStack-ceph/connect16.png)

- ceph의 Block > 이미지 에 들어가보면 아까 images말고도 volumes 데이터풀이 아까 지정한 5GB로 할당된 것을 볼 수 있다.

![connect17](assets/04-connect-openStack-ceph/connect17.png)
