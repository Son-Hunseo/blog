...
---
> [!info] DNS에 대한 개념이 없다면, [DNS](../../../../05-CS/03-Network/02-dns.md) 글 참고
...
- `kubelet`
	- `kubelet`이 새로운 `Pod`가 생성될 때 `Pod`의 `/etc/resolv.conf` 파일의 `nameserver`로 `CoreDNS` 서비스의 IP를 기입한다. (참고 [네임서버 설정](../../../../05-CS/03-Network/02-dns.md#dns-서버-설정))
- `kube-apiserver`
