...
이전 [`Ingress`글에서의 예시](23-ingress.md#ingress-설정-예시)를 생각해보자.

- `wear-service`와 `video-service`가 각각 다른 팀에서 관리한다고 했을 때(멀티 테넌시 환경) 같은 파일을 수정하므로, 충돌이 발생할 위험이 높다.
...
> [!info] [`Ingress`글에서의 경로 기반 예시](23-ingress.md#경로-기반)
...
- 효과
	- `GatewayClass` 타입에 대한 권한을 인프라 관리자에게만 줌으로써 다른 직무의 팀원들이 해당 설정을 변경할 여지를 줄인다.
	- 다른 직무의 팀원들이 `GatewayClass`의 벤더(구현체)가 무엇인지 알 필요도 없다. (하위 리소스들들의 설정에서 벤더 종속성이 없어지기 때문)
...
- 효과 
	- 해당 `Gateway` 객체에 대한 권한을 해당 서비스를 관리하는 클러스터 운영자에게만 부여한다. 또한, `exposed: "true"` 레이블이 붙은 네임스페이스의 Route만 허용함으로써 허용되지 않은 다른 팀이 해당 호스트네임 하위 경로를 사용하는 것을 방지한다.
