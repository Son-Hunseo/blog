...
- `HPA`와 달리 `VPA`는 <span class="t-red">기본적으로 내장되어있지 않아 따로 설치해야한다.</span>
- <span class="t-red">Metric Server 필수</span>
...
> [!tip] Q) 위처럼 `Pod`를 1개씩 스케일하며, 1개일 경우 evict를 차단한다면 애플리케이션이 작동하지 않는 기간이 발생하지 않는 것 아닌가요?
> A) 하지만, 구조적으로 eviction을 강제하는 설정을 하거나, 사용자 혹은 컨트롤러가 `Pod`를 재시작할 타이밍에 우연히 `VPA`가 동시에 작동하는 등의 예외적인 상황이 존재하므로 '구조적으로' 다운타임이 발생할 '여지'가 있다고 보는 것이 맞다.
...
1. (In-place pod vertical scaling 기능 적용 전) Pod를 삭제하고 새로 만들어야함.
2. HPA와의 충돌 문제

> 결국, 대부분의 조직이 VPA를 현재 워크로드가 사용하는 리소스를 측정하기 위한 Recommender 모드 '계산기'로 사용한다.
