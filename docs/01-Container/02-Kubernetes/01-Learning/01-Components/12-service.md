...
1. `Service`는 애플리케이션 내부 구성 요소(`Pod`)와 외부 구성 요소 간 통신을 가능하게 한다.
...
2. `Service`는 애플리케이션을 다른 애플리케이션과 연결하는데 도움을 준다.
...
3. `Service`는 애플리케이션 내의 마이크로서비스 간의 느슨한 결합(Loose Coupling)을 가능하게 한다.
...
![service1](assets/12-service/service1.png)
...
![service2](assets/12-service/service2.png)
...
![service3](assets/12-service/service3.png)
...
![service4](assets/12-service/service4.png)
...
- 하지만, 이는 이를 지원하는 클라우드 플랫폼에서만 가능하다.(`MetalLB` 같은 대안도 존재하긴한다) 지원하지 않는 플랫폼에서 구성한다면 `NodePort`와 다를 것이 없으며 `Node` 레벨의 로드밸런싱 기능도 수행하지 않는다.
