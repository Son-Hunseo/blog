---
image: /img/posts/03-Dev/31-Project/06-Chocoletter-Advance-06/springevent1.png
...
> [!info] 메시지 큐를 도입하지 않고 `Spring Event`를 사용한 이유는 현재 서비스는 MSA 환경이 아닌 하나의 서버에서 모든 로직을 처리하는 간단한 모놀리틱 아키텍처 서버이다. 이러한 환경에서 도메인 간의 결합도를 낮추기 위해 메시지 큐를 도입하는 것은 더 많은 관리 포인트가 추가되는 오버엔지니어링이라고 판단했다.
